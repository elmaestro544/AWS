  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
        try {
            ...
            // 1. Initial Check - Do this first to avoid race conditions
            ...
            // Ensure we ALWAYS clear the loading state
            setIsAuthChecking(false);
        }

        // 2. Subscription - Set up listener after initial check
        const { data } = supabase.auth.onAuthStateChange(async (event, session) => {
            ...
            // We intentionally ignore INITIAL_SESSION here because we handled it manually above
            // to ensure isAuthChecking is set to false reliably.
        });
