    ...
    React.createElement('div', { className: "flex justify-center flex-wrap gap-2 mb-4" },
      PARAPHRASE_MODES.map(mode => (
        React.createElement('button', {
          ...
          className: `px-4 py-2 text-sm font-semibold rounded-full transition-colors ${selectedMode === mode.id ? 'bg-brand-red text-white' : 'bg-slate-200 dark:bg-card-gradient text-slate-700 dark:text-brand-text-light hover:bg-slate-300 dark:hover:bg-brand-blue/50'}`
        }, mode.name(language))
      ))
    ),
    ...
    React.createElement('div', { className: "grid grid-cols-1 md:grid-cols-2 gap-4" },
      ...
      React.createElement('div', { className: "flex flex-col gap-4" },
        React.createElement('textarea', {
          ...
          className: "w-full h-64 md:h-80 p-3 bg-white dark:bg-input-gradient border border-slate-300 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-brand-blue focus:outline-none resize-none"
        }),
        React.createElement('button', {
      ...
      // Output Panel
      React.createElement('div', { className: "relative w-full h-64 md:h-96 bg-white dark:bg-card-gradient border border-slate-200 dark:border-white/10 rounded-xl flex flex-col" },
        isLoading ? (
          ...
          React.createElement(React.Fragment, null,
            ...
            paraphrasedSuggestions.length > 1 && React.createElement('div', { className: 'flex justify-between items-center p-2 border-t border-slate-200 dark:border-white/10 flex-shrink-0' },
                React.createElement('button', {
                    ...
                    className: "flex items-center gap-1 text-sm font-semibold text-slate-600 dark:text-brand-text-light disabled:opacity-50 hover:text-brand-red dark:hover:text-brand-red"
