...
const Home = ({ language, onNavigateToServices }) => {
            ...
            React.createElement('div', { className: 'grid md:grid-cols-2 gap-12 items-center' },
                React.createElement('div', { className: `text-center md:text-left ${language === 'ar' ? 'md:text-right' : ''}` },
                    ...
                    React.createElement('div', { className: `mt-8 flex justify-center md:justify-start gap-4 ${language === 'ar' ? 'md:justify-end' : ''}` },
                        ...
                        React.createElement('button', {
                            className: 'px-8 py-3 bg-slate-200 dark:bg-dark-card text-slate-800 dark:text-white font-semibold rounded-lg hover:bg-slate-300 dark:hover:bg-opacity-80 transition-colors'
                ...
                React.createElement('div', { className: 'relative h-80 md:h-full flex items-center justify-center' },
                    ...
                    React.createElement('div', { className: 'relative animate-float' },
                        React.createElement('div', { className: 'w-72 h-72 bg-white/50 dark:bg-dark-card/50 backdrop-blur-md rounded-2xl transform rotate-45' }),
                        React.createElement('div', { className: 'absolute inset-4 bg-white/80 dark:bg-dark-card/80 rounded-lg transform -rotate-45 flex items-center justify-center' },
    ...
    const WhatWeDoSection = () => {
        ...
        return React.createElement('section', { className: 'bg-slate-100 dark:bg-dark-card py-20' },
                ...
                React.createElement('div', { className: 'grid md:grid-cols-3 gap-8 mt-12' },
                    services.map((service, index) => 
                        React.createElement('div', { 
                            key: index, 
                            className: 'bg-white dark:bg-dark-bg p-8 rounded-lg shadow-lg text-center transform hover:-translate-y-2 transition-transform'
                        },
            ...
            React.createElement('div', { className: 'container mx-auto px-6' },
                ...
                React.createElement('div', { className: 'grid md:grid-cols-2 gap-8' },
                    projects.map((src, index) =>
                        React.createElement('div', { key: index, className: 'group relative overflow-hidden rounded-lg shadow-lg' },
                            ...
                            React.createElement('div', { className: 'absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center' },
            ...
            React.createElement('div', { className: 'container mx-auto px-6' },
                ...
                React.createElement('div', { className: 'grid md:grid-cols-3 gap-8' },
                    team.map((member, index) => 
                        React.createElement('div', { key: index, className: 'text-center bg-white dark:bg-dark-bg p-6 rounded-lg shadow-lg' },
                            React.createElement('img', { src: member.img, alt: member.name, className: 'w-32 h-32 rounded-full mx-auto mb-4 border-4 border-brand-red' }),
    ...
    return React.createElement('div', { className: "bg-white dark:bg-dark-bg" },
