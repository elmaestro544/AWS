      ...
      React.createElement('div', { className: 'hidden md:flex items-center gap-4' },
        ...
        isAuthenticated ? (
          React.createElement('div', { className: "relative" },
            ...
            isUserMenuOpen && React.createElement('div', { className: "absolute top-full right-0 mt-2 w-32 bg-white dark:bg-dark-card border border-slate-200 dark:border-white/10 rounded-md shadow-lg z-20" },
...
const App = () => {
  ...
  useEffect(() => {
    ...
    try {
        const userJson = localStorage.getItem('scigenius_user');
        if (userJson) {
        ...
        // If storage is blocked, show welcome modal once per page load.
        setIsWelcomeModalOpen(true);
    }
  }, []);
  ...
  const handleLoginSuccess = (user) => {
    try {
        localStorage.setItem('scigenius_user', JSON.stringify(user));
        setCurrentUser(user);
  ...
  const handleLogout = () => {
    try {
        localStorage.removeItem('scigenius_user');
        setCurrentUser(null);
    } catch (error) {
