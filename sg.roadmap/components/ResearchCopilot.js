...
const PaperCard = ({ paper, language }) => (
  React.createElement('div', { className: "bg-white dark:bg-card-gradient p-4 rounded-xl border border-slate-200 dark:border-white/10 transition-all duration-300 hover:border-brand-blue/50 hover:shadow-glow-blue" },
...
const ResearchCopilot = ({ language }) => {
  ...
  return React.createElement('div', { className: "flex flex-col h-[calc(100vh-200px)] max-w-4xl mx-auto" },
    ...
    file && (
      React.createElement('div', { className: "flex flex-col flex-grow bg-white dark:bg-card-gradient rounded-2xl overflow-hidden border border-slate-200 dark:border-white/10" },
        React.createElement('div', { ref: chatContainerRef, className: "flex-grow p-4 space-y-4 overflow-y-auto" },
          messages.length === 0 && !isUploading && (
            React.createElement('div', { className: "text-center p-4 bg-slate-200 dark:bg-black/20 rounded-lg" },
              ...
              React.createElement('div', { className: `grid grid-cols-1 sm:grid-cols-3 gap-2 mt-4 ${language === Language.AR ? 'sm:grid-flow-col-dense' : ''}` },
                [t.suggestion1, t.suggestion2, t.suggestion3].map(suggestion => {
                  const buttonProps = {
                    ...
                    className: "bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-slate-700 dark:text-brand-text py-2 px-3 rounded-full text-sm transition-colors"
                  };
          ...
          messages.map((msg, index) => (
            React.createElement('div', { key: index, className: `flex ${msg.sender === 'user' ? (language === Language.AR ? 'justify-start' : 'justify-end') : (language === Language.AR ? 'justify-end' : 'justify-start')}` },
              React.createElement('div', { className: `relative max-w-lg p-3 rounded-2xl ${msg.sender === 'user' ? 'bg-brand-blue text-white' : 'bg-slate-200 dark:bg-card-gradient text-slate-800 dark:text-brand-text'}` },
        ...
        React.createElement('div', { className: "p-4 border-t border-slate-200 dark:border-white/10" },
          React.createElement('div', { className: `relative flex items-center ${language === Language.AR ? 'flex-row-reverse' : ''}` },
            React.createElement('input', {
              ...
              className: "w-full bg-white dark:bg-input-gradient border border-slate-300 dark:border-white/20 rounded-full py-2.5 focus:ring-2 focus:ring-brand-blue focus:outline-none transition-all placeholder-slate-400 dark:placeholder-slate-500",
