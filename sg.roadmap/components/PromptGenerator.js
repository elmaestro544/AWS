    ...
    return React.createElement('div', null,
        ...
        React.createElement('div', { className: "space-y-4" },
            React.createElement('textarea', {
                ...
                className: "w-full h-24 p-3 bg-white dark:bg-input-gradient border border-slate-300 dark:border-white/20 rounded-xl focus:ring-2 focus:ring-brand-blue focus:outline-none transition-colors placeholder-slate-400 dark:placeholder-slate-500",
                disabled: isLoading
            }),
            React.createElement('div', { className: "grid grid-cols-2 sm:grid-cols-4 gap-4" },
                promptTypes.map(({ type, label, loading }) => (
                    React.createElement('button', {
                        ...
                        className: "w-full flex justify-center items-center bg-brand-red hover:bg-red-500 disabled:bg-slate-400 dark:disabled:bg-slate-600 disabled:cursor-not-allowed text-white font-bold py-3 px-4 rounded-full transition-colors"
        ...
        (isLoading || generatedPrompt) && (
            React.createElement('div', { className: "mt-8" },
                React.createElement('div', { className: "relative bg-white dark:bg-card-gradient p-4 border border-slate-200 dark:border-white/10 rounded-xl" },
                    !generatedPrompt && isLoading ? (
                        ...
                        React.createElement(React.Fragment, null, 
                            React.createElement('div', { className: `absolute top-2.5 ${language === 'ar' ? 'left-2.5' : 'right-2.5'} flex items-center gap-2` },
                                React.createElement('button', { onClick: () => handleListen(generatedPrompt), disabled: isAudioLoading, className: 'flex items-center justify-center h-7 w-7 bg-slate-200 dark:bg-white/10 hover:bg-slate-300 dark:hover:bg-white/20 p-1.5 rounded-full disabled:opacity-50'}, isAudioLoading ? React.createElement(Spinner, {size: '4'}) : (isSpeaking ? React.createElement(StopIcon, {className: "h-4 w-4"}) : React.createElement(SpeakerIcon, {className: "h-4 w-4"}))),
    ...
    React.createElement('div', null,
        ...
        React.createElement('div', { className: "space-y-4" },
            React.createElement('textarea', {
                ...
                className: "w-full h-32 p-3 bg-white dark:bg-input-gradient border border-slate-300 dark:border-white/20 rounded-xl focus:ring-2 focus:ring-brand-blue focus:outline-none transition-colors placeholder-slate-400 dark:placeholder-slate-500",
                disabled: isChecking
            }),
            React.createElement('button', {
        ...
        analysisResult && !analysisResult.error && (
            React.createElement('div', { className: "mt-8 space-y-6" },
                React.createElement('div', { className: "bg-white dark:bg-card-gradient p-4 border border-slate-200 dark:border-white/10 rounded-xl" },
                ...
                React.createElement('div', { className: "relative bg-white dark:bg-card-gradient p-4 border border-slate-200 dark:border-white/10 rounded-xl" },
                    React.createElement('h3', { className: "font-bold text-lg text-slate-900 dark:text-brand-text mb-2" }, t.enhancedPromptTitle),
                     React.createElement('div', { className: `absolute top-2.5 ${language === 'ar' ? 'left-2.5' : 'right-2.5'} flex items-center gap-2` },
                        React.createElement('button', { onClick: () => handleListen(analysisResult.enhancedPrompt), disabled: isAudioLoading, className: 'flex items-center justify-center h-7 w-7 bg-slate-200 dark:bg-white/10 hover:bg-slate-300 dark:hover:bg-white/20 p-1.5 rounded-full disabled:opacity-50'}, isAudioLoading ? React.createElement(Spinner, {size: '4'}) : (isSpeaking ? React.createElement(StopIcon, {className: "h-4 w-4"}) : React.createElement(SpeakerIcon, {className: "h-4 w-4"}))),
