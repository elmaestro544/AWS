...
import { generateAnswerShortQuestion, scoreAnswerShortQuestionAttempt } from '../services/geminiService.js';
...
const AnswerShortQuestionPractice = ({ navigateTo }) => {
    ...
    const fetchNewQuestion = useCallback(async () => {
        ...
        try {
            const { question, answer, audioBase64 } = await generateAnswerShortQuestion();
            setQuestionData({ question, answer });
    ...
    const handleSubmit = async () => {
        ...
        setScore(null);
        try {
            const analysis = await scoreAnswerShortQuestionAttempt(questionData.answer);
            setScore(analysis);
    ...
    return React.createElement('div', { className: "max-w-4xl mx-auto" },
        ...
        React.createElement('h1', { className: "text-3xl font-bold text-gray-900" }, 'Answer Short Question'),
        React.createElement('p', { className: "text-gray-600 my-2" }, 'You will hear a question. Please give a simple and short answer.'),
        ...
        React.createElement('div', { className: "mt-6 bg-white p-8 rounded-xl shadow-lg flex flex-col items-center justify-center min-h-[150px]" },
            isLoading ? (
                ...
                React.createElement('button', { onClick: playQuestionAudio, disabled: isPlaying || isRecording, className: "flex items-center space-x-3 px-6 py-3 bg-blue-500 text-white rounded-full shadow-lg hover:bg-blue-600 transition-transform transform hover:scale-105 disabled:bg-gray-400 disabled:cursor-not-allowed" },
                    React.createElement(PlayCircleIcon, { className: "w-8 h-8" }),
                    React.createElement('span', { className: "text-lg font-semibold" }, isPlaying ? 'Playing...' : 'Play Question')
                )
            )
        ),
        ...
        isAnswerVisible && (
            React.createElement('div', { className: "mt-8 bg-gray-100 p-4 rounded-lg text-center" },
                React.createElement('h3', { className: "font-semibold text-gray-800" }, 'Question:'),
                React.createElement('p', { className: "text-gray-700 italic mt-1" }, questionData.question),
                React.createElement('h3', { className: "font-semibold text-gray-800 mt-3" }, 'Correct Answer:'),
                React.createElement('p', { className: "text-green-700 font-bold text-lg mt-1" }, questionData.answer)
            )
        ),
