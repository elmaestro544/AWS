...
import { generateHighlightCorrectSummaryQuestion } from '../services/geminiService.js';
...
const HighlightCorrectSummaryPractice = ({ navigateTo }) => {
    ...
    const fetchNewQuestion = useCallback(async () => {
        ...
        try {
            const data = await generateHighlightCorrectSummaryQuestion();
    ...
    const renderOptions = () => {
        ...
        return questionData.options.map((option, index) => {
            ...
            let labelClass = "flex items-start p-4 rounded-lg border cursor-pointer transition-colors duration-150 ";
            if (isSubmitted) {
                 if (isCorrect) {
    ...
    const renderResult = () => {
        ...
        return React.createElement('div', { className: "mt-8 bg-white p-6 rounded-xl shadow-lg text-center animate-fade-in" },
            React.createElement('h2', { className: "text-2xl font-bold text-gray-800" }, isCorrect ? "Correct!" : "Incorrect"),
            React.createElement('p', { className: `mt-2 text-xl ${isCorrect ? 'text-green-600' : 'text-red-600'}` }, 
                isCorrect ? "You selected the correct summary." : "You did not select the correct summary."
            ),
             React.createElement('div', { className: "mt-4 text-left bg-gray-50 p-4 rounded-lg" },
                React.createElement('h3', { className: "font-semibold mb-2 text-gray-800" }, "Correct Summary:"),
                React.createElement('p', { className: "text-green-700" }, questionData.correctAnswer)
            )
        );
    };
    ...
    return React.createElement('div', { className: "max-w-4xl mx-auto" },
        ...
        React.createElement('h1', { className: "text-3xl font-bold text-gray-900" }, 'Highlight Correct Summary'),
        React.createElement('p', { className: "text-gray-600 my-2" }, 'You will hear a recording. Choose the paragraph that best summarizes the recording.'),
            ...
            isLoading ? (
                ...
                React.createElement(React.Fragment, null,
                    ...
                    React.createElement('div', { className: "mt-6 pt-6 border-t" },
                        React.createElement('p', { className: 'text-lg font-semibold text-gray-800 mb-4' }, 'Please select the correct summary:'),
        ...
        React.createElement('div', { className: "mt-6 flex flex-col sm:flex-row items-center justify-center gap-4" },
             React.createElement('button', {
                ...
                disabled: isLoading || isSubmitted || !selectedAnswer,
